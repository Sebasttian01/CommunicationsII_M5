...
# Description: Let you to see not only the eye diagram but also let to make the timing visually and manually. But not only, it also makes the sampling. The key parameter is Retardo_Timing
...
class b_EYE_Timing_sampler_f(gr.hier_block2, Qt.QWidget):
    def __init__(self, AlphaLineas=0.5, Delay=0, GrosorLineas=20, N_eyes=2, Samprate=1000, Sps=64, Title="Eye Diagram and Timing", Ymax=2, Ymin=(-2)):
        gr.hier_block2.__init__(
            self, "b_EYE_Timing_sampler_f",
        ...
        # Variables
        ##################################################
        self.Retardo_Timing = Retardo_Timing = 7

        ##################################################
        # Blocks
        ##################################################

        self._Retardo_Timing_range = qtgui.Range(0, (Sps-1), 1, 7, 200)
        self._Retardo_Timing_win = qtgui.RangeWidget(self._Retardo_Timing_range, self.set_Retardo_Timing, "Retardo_Timing", "counter_slider", int, QtCore.Qt.Horizontal)
        self.top_grid_layout.addWidget(self._Retardo_Timing_win, 0, 0, 1, 1)
        for r in range(0, 1):
