...
# Title: b_RNyquist_Filter_cc
# Author: Done by: Homero Ortega Boada. Universidad Industrial de Santander
# Description: It is the Root Nyquist Filter in a proof period. I means that that two of this blocks, for example one on the transmitter and one on the receiver form a Nyquist Filter. Parameters: Sps - the number of samples per simbol the filter will produce; ntaps- the number of components in the impulse response
...
class b_RNyquis_Filter_cc(gr.hier_block2):
    def __init__(self, Ganancia=1., Sps=8, ntaps=16):
        gr.hier_block2.__init__(
            self, "b_RNyquist_Filter_cc",
        ...
        self.taps = taps = numpy.sqrt(numpy.sinc(n_vector/Sps))
    ...
    def set_Sps(self, Sps):
        self.Sps = Sps
        self.set_taps(numpy.sqrt(numpy.sinc(self.n_vector/self.Sps)))
