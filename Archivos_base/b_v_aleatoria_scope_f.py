...
# Description: Some parameters of a random variable can visualized, like: the mean, the RMS value, the mean power, the PSD, the time signal. Parameters used: samp_rate - the sample rate of the signal; N_time - the number of time samples to be shown; N_frec- the number of frequency pointss to be shown
...
from b_QT_autocorrel import b_QT_autocorrel  # grc-generated hier_block
...
class b_v_aleatoria_scope_f(gr.hier_block2, Qt.QWidget):
    def __init__(self, N_frec=2014, N_time=2014, V_p=2014, samp_rate=22000):
        gr.hier_block2.__init__(
            self, "b_v_aleatoria_scope_f",
        ...
        self.top_grid_layout.addWidget(self._qtgui_histogram_sink_x_0_win, 1, 2, 1, 2)
        for r in range(1, 2):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(2, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.b_QT_autocorrel_0 = b_QT_autocorrel(
            Ensayos=1000000,
            N=N_frec,
            Ymax=4e-7,
        ...
        self.top_grid_layout.addWidget(self.b_QT_autocorrel_0, 2, 2, 1, 2)
        for r in range(2, 3):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(2, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.top_grid_layout.addWidget(self.b_PSD_0, 2, 0, 1, 2)
        for r in range(2, 3):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.connect((self, 0), (self.b_PSD_0, 0))
        self.connect((self, 0), (self.b_QT_autocorrel_0, 0))
        ...
        self.connect((self, 0), (self.qtgui_histogram_sink_x_0, 0))
    ...
    def set_N_frec(self, N_frec):
        self.N_frec = N_frec
        self.b_PSD_0.set_N(self.N_frec)
        self.b_QT_autocorrel_0.set_N(self.N_frec)
    ...
    def set_V_p(self, V_p):
        self.V_p = V_p
        self.qtgui_histogram_sink_x_0.set_x_axis((-self.V_p), self.V_p)
    ...
    def set_samp_rate(self, samp_rate):
        self.samp_rate = samp_rate
        self.b_PSD_0.set_Ymax(1./self.samp_rate)
        self.b_QT_autocorrel_0.set_samp_rate_audio(self.samp_rate)
